...
get_ipython().run_line_magic('timeit', 'S = np.hstack(pool.map(mcs_simulation_np, p * [(M, int(I / p))]))')


# In[ ]:


get_ipython().run_line_magic('timeit', 'S = np.hstack(pool.map(mcs_simulation_nb, p * [(M, int(I / p))]))')


# In[ ]:


get_ipython().run_line_magic('timeit', 'S = np.hstack(pool.map(mcs_simulation_cy, p * [(M, int(I / p))]))')
