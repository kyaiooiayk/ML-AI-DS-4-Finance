...
for j in liq_vol_all.TICKER.unique():
    for i in range(len(liq_vol_all[liq_vol_all.TICKER == j])):
        liq_ratio.append(
            (
                liq_vol_all["PRC"][i + 1 : i + 6] * liq_vol_all["VOL"][i + 1 : i + 6]
            ).sum()
            / (
                np.abs(
                    liq_vol_all["PRC"][i + 1 : i + 6].mean()
...
for j in liq_vol_all.TICKER.unique():
    for i in range(len(liq_vol_all[liq_vol_all.TICKER == j])):
        Lhh.append(
            (liq_vol_all["PRC"][i : i + 5].max() - liq_vol_all["PRC"][i : i + 5].min())
...
liq_vol_all["percent_quoted_ba"] = (
    liq_vol_all.ASKHI - liq_vol_all.BIDLO
) / liq_vol_all.mid_price
liq_vol_all["percent_effective_ba"] = (
    2 * abs((liq_vol_all.PRC - liq_vol_all.mid_price)) / liq_vol_all.mid_price
)
...
liq_vol_all["price_diff"] = liq_vol_all.groupby("TICKER")["PRC"].apply(
    lambda x: x.diff()
)
    ...
    for i in range(len(liq_vol_all[liq_vol_all.TICKER == j])):
        gamma.append(
            (
                ...
                - min(liq_vol_all["BIDLO"].iloc[i + 1], liq_vol_all["BIDLO"].iloc[i])
            )
            ** 2
        )
...
alpha = (
    (np.sqrt(2 * beta_array) - np.sqrt(beta_array)) / (3 - (2 * np.sqrt(2)))
) - np.sqrt(gamma_array / (3 - (2 * np.sqrt(2))))
...
for j in liq_vol_all.TICKER.unique():
    for i in range(len(liq_vol_all[liq_vol_all.TICKER == j])):
        dvol.append(
            (liq_vol_all["PRC"][i : i + 5] * liq_vol_all["VOL"][i : i + 5]).sum()
        )
...
liq_vol_all["vol_diff_pct"] = (
    liq_vol_all.groupby("TICKER")["VOL"].apply(lambda x: x.diff()).pct_change()
)
liq_vol_all["price_diff_pct"] = (
    liq_vol_all.groupby("TICKER")["PRC"].apply(lambda x: x.diff()).pct_change()
)
...
for j in liq_vol_all.TICKER.unique():
    for i in range(len(liq_vol_all[liq_vol_all.TICKER == j])):
        cet.append(
            np.sum(liq_vol_all["vol_diff_pct"][i : i + 1])
...
liq_vol_all["VOL_pct_change"] = liq_vol_all.groupby("TICKER")["VOL"].apply(
...
for i, j in zip(
    liq_vol_all.TICKER.unique(), range(len(liq_vol_all["TICKER"].unique()))
):
...
clusters = [
    GaussianMixture(n, covariance_type="spherical", random_state=0).fit(scaled_liq)
...
def cluster_state(data, nstates):
    gmm = GaussianMixture(
        n_components=nstates, covariance_type="spherical", init_params="kmeans"
    )
    ...
    state_probs_df = pd.DataFrame(
        state_probs, columns=["state-1", "state-2", "state-3"]
    )
    state_prob_means = [
        state_probs_df.iloc[:, i].mean() for i in range(len(state_probs_df.columns))
    ]
    if np.max(state_prob_means) == state_prob_means[0]:
        print(
            "State-1 is likely to occur with a probability of {:4f}".format(
                state_prob_means[0]
            )
        )
    elif np.max(state_prob_means) == state_prob_means[1]:
        print(
            "State-2 is likely to occur with a probability of {:4f}".format(
                state_prob_means[1]
            )
        )
    else:
        print(
            "State-3 is likely to occur with a probability of {:4f}".format(
                state_prob_means[2]
            )
        )
    return state_probs
...
loading_matrix = pd.DataFrame(
    loadings, columns=["PC1", "PC2", "PC3"], index=liq_measures_all.columns
)
loading_matrix
