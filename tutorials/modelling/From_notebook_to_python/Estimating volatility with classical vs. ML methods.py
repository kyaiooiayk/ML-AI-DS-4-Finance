...
for p in range(1, 5):
    for q in range(1, 5):
        ...
        if garch.bic == np.min(bic_garch):
            best_param = p, q
garch = arch_model(
    ret, mean="zero", vol="GARCH", p=best_param[0], o=0, q=best_param[1]
).fit(disp="off")
print(garch.summary())
...
for p in range(1, 5):
    for q in range(1, 5):
        ...
        if gjrgarch.bic == np.min(bic_gjr_garch):
            best_param = p, q
gjrgarch = arch_model(ret, mean="zero", p=best_param[0], o=1, q=best_param[1]).fit(
    disp="off"
)
...
plt.plot(realized_vol / 100, label="Realized Volatility")
plt.plot(
    forecast_gjrgarch.variance.iloc[-len(split_date) :] / 100,
...
for p in range(1, 5):
    for q in range(1, 5):
        ...
        if egarch.bic == np.min(bic_egarch):
            best_param = p, q
egarch = arch_model(
    ret, mean="zero", vol="EGARCH", p=best_param[0], q=best_param[1]
).fit(disp="off")
...
clf.fit(
    X.iloc[:-n].values,
    realized_vol.iloc[1 : -(n - 1)].values.reshape(
        -1,
    ),
)
...
for i, j, k in zip(range(4), epochs_trial, batch_trial):
    ...
    DL_RMSE.append(
        np.sqrt(mse(realized_vol.iloc[-n:] / 100, DL_predict.flatten() / 100))
    )
...
P = [[0.5, 0.2, 0.3], [0.2, 0.3, 0.5], [0.2, 0.2, 0.6]]
...
bayesian_RMSE = np.sqrt(
    mse(realized_vol.iloc[-n:] / 100, bayesian_prediction.values / 100)
)
